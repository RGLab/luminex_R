%\VignetteIndexEntry{Extracting Raw bead level data using MasterPlex CT}
%\VignetteDepends{LumiR}
%\VignetteKeywords{Luminex multiplex assay analysis}
%\VignettePackage{LumiR}
\documentclass[11pt]{article}
\usepackage{Sweave}
\usepackage{hyperref}
\usepackage{underscore}
\usepackage[hmargin=2cm, vmargin=3cm]{geometry}
\SweaveOpts{keep.source=FALSE}

\title{The LumiR User Guide}
\author{Renan Sauteraud\footnote{rsautera@fhcrc.org}}

\begin{document}
\maketitle

\tableofcontents

%\newpage
\section{General}
Classical statistical analysis of xMap, and notably MiraiBio's own analysis software, MasterPlex \textbf{QT}, only uses the MFI.


NiraiBio's acquisition software for bead array multiplex assay, MasterPlex CT, outputs raw bead level data in binary files with a \textbf{.lxb} extension.

\section{Bead level}
In the first version of xPONENT, the filenames are \textbf{Run[001-096].csv}. Only these files should be included in the experiment folder, in a directory using the plate name.
There is also a file \textbf{Output.csv} that should be removed from the experiment folder when using \texttt{LumiR}


\section{Analyte mapping}
Acquisition setup -> Output -> Save binary file also.

When using MiraiBio's analysis software, 


\end{document}
