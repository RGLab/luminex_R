...
read.experiment<-function(path="./"){
  ...
  if(length(pheno.file)==0){#Get plate/file/well based on the structure of the folder
    ...
    phenoData<-.read.pheno.xPonent(pheno.file)
  }
...
.getXponentWellsID<-function(filenames){#Run001.csv
  wellsNo<-as.numeric(gsub("Run|.csv", "", filenames))
...
.read.pheno.xPonent<-function(pheno.file){
